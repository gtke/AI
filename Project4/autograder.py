...
def q1Test(testFile,module=None):
    ...
    if 'Pertinent' in testFile.name:
        ...
        solution = module.getClassCounts(examples, className).values()
...
def sameListContents(list1,list2,floatDif= 0.00001):
    if len(list1)!=len(list2):
        return False
    for index in xrange(len(list1)):
        for index2 in xrange(len(list2)):
            if isinstance(list1[index],float):
                if abs(list1[index]-list2[index2])>floatDif:
                    list2.pop(index2)
                    break
            elif isinstance(list1[index],list) and isinstance(list2[index2],list):
                if sameListContents(list1[index],list2[index2]):
                    list2.pop(index2)
                    break
...
def sameList(list1,list2,floatDif= 0.00001):
    if len(list1)!=len(list2):
        return False
    for index in xrange(len(list1)):
        if isinstance(list1[index],float):
    ...
    for fileName in [name for name in listdir('test_cases/'+q) if 'test' in name]:
        ...
        if isinstance(solution,float):
            ...
            if ordered:
                ...
                solFile.write('correct=sameListContents(result,%s,%f);\n%s'%(str(solution),floatDif,str(solution)))
        else:
